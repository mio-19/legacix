...
This device support reuses parts of postmarketOS work for `google-trogdor` /
`google-wormdingler`, notably the SC7180 kernel baseline/config choices and
accelerometer behavior quirks.
...
in the package set as `lenovo-wormdingler-unredistributable-firmware`.
...
{ /* configuration.nix */
  hardware.firmware = [
    pkgs.lenovo-wormdingler-unredistributable-firmware
  ];
}
----
